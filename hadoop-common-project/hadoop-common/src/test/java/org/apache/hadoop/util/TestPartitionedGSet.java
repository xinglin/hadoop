...
public class TestPartitionedGSet {
  ...
  /**
   * Generate positive random numbers for testing. We want to use only positive numbers because the smallest
   * partition used in testing is 0.
   *
   * @param length
  ...
  protected ReentrantReadWriteLock topLock = new ReentrantReadWriteLock(false);
  /**
  ...
  public void testIteratorForNoPartition() {
    PartitionedGSet<TestElement, TestElement> set =
        new PartitionedGSet<TestElement, TestElement>(16, new TestElementComparator(), new NoOpLock());
  ...
  /**
   * Test whether the iterator can return the same number of elements as stored into the PartitionedGSet.
   */
  @Test(timeout=60000)
  public void testIteratorCountElements() {
    ...
    topLock.readLock().unlock();
    Assert.assertEquals(count, ELEMENT_NUM);
  }
